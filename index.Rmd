...
**Future work**
  - Use our proposed models for the inverse prediction problem `r Citet(bib, "Parnell2015")`:
    Given new pollen counts $\mathbf{Y}_i^\ast$ what is the associated climate $\mathbf{x}^{\ast}$ associated with them, i.e., $p(\mathbf{x}^{\ast} \mid \mathbf{Y}_i^\ast, \mathbf{Y}, \mathbf{x})$?.
